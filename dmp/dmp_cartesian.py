...
class DMPs_cartesian(object):
	...
	def step(self, tau = 1.0, error = 0.0, external_force = None,
		...
		#add random noise movement if local minima is detected
		#local minima is detected based on heuristic; if velocity is
		#small and not close to goal, then it's at local minima.
		min_vel=0.01
		dgoal=np.linalg.norm(self.x-self.x_goal)
		near_goal=dgoal<0.03
		impulse_mag=0.05
		if np.linalg.norm(self.dx)<min_vel and not near_goal:
			
			self.ddx+=0.1*np.random.random_sample((3,))-np.array([0.05,0.05,0.05])
